...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output.f.mul_zip(sigmoid, grad_output) - grad_output.f.mul_zip(
            grad_output.f.mul_zip(sigmoid, sigmoid), grad_output
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order = tuple(int(i) for i in order.to_numpy().flatten())
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        return grad_output._new(
            grad_output._tensor.permute(*np.argsort(order))
        ), 0.0
